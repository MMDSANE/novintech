...
(function ($) {
	...
	if ($('body').hasClass('tt-transition')) {
		...
		function HideLoad() {
			...
			if ($pciTitle.length) {
				tl_transitOut.fromTo(
					$pciTitle,
					{ autoAlpha: 0, x: 100, scaleX: 1.3, transformOrigin: 'right' },
					{
						duration: 1.5,
						autoAlpha: 1,
						x: 0,
			...
			if ($pciCategory.length) {
				tl_transitOut.fromTo(
					$pciCategory,
					{ autoAlpha: 0, x: 60, scaleX: 1.3, transformOrigin: 'right' },
					{
						duration: 1.5,
						autoAlpha: 1,
						x: 0,
	...
	if ($('body').hasClass('tt-smooth-scroll')) {
		...
		if (!isMobile) {
			...
			// AnchorPlugin (URL with hash links load in the left position)
	...
	$('.lightgallery').lightGallery({
		...
		facebookDropdownText: 'فیس بوک', // Facebok dropdown text.
		twitter: true, // Enable Twitter share.
		twitterDropdownText: 'توییتر', // Twitter dropdown text.
		googlePlus: true, // Enable Google Plus share.
		googlePlusDropdownText: 'گوگل پلاس', // Google Plus dropdown text.
		pinterest: true, // Enable Pinterest share.
		pinterestDropdownText: 'پینترست', // Pinterest dropdown text.
	});
		...
		if ($('#page-header').hasClass('ph-bg-image')) {
			if ($('.ph-caption').length) {
				if ($('#page-header').hasClass('ph-center')) {
					...
					tlPhParallax.to(
						'.ph-caption',
						{ y: 180, opacity: 0, scale: 0.95, transformOrigin: 'right' },
						0
					);
				}
			}
	...
	if ($('.tt-sliding-sidebar-wrap').length) {
		$('body').addClass('tt-sliding-sidebar-on tt-sliding-sidebar-right-on'); // right position is by default.
		...
		// If sliding sidebar has class "tt-ss-left" add class "tt-ss-left-on" to <pody>.
		if ($('.tt-sliding-sidebar-wrap').hasClass('tt-ss-left')) {
			$('body').removeClass('tt-sliding-sidebar-right-on');
			$('body').addClass('tt-sliding-sidebar-left-on');
		}
	}
